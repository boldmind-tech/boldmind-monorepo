...
let TrendService = TrendService_1 = class TrendService {
    constructor(gemini, redis) {
        this.gemini = gemini;
        this.redis = redis;
        this.logger = new common_1.Logger(TrendService_1.name);
        this.CACHE_TTL = 1800;
    }
    async getTrendingForBoldMind() {
    ...
    async getTrendingTechUpdates() {
        ...
        if (this.gemini.available) {
            try {
                const result = await this.gemini.searchAndSummarize(`What are the top 10 trending technology and startup news topics in Nigeria and Africa today? 
          ...
          Return ONLY the JSON array, no markdown.`, 'You are a Nigerian tech news curator. Find the most relevant trending topics for Nigerian tech entrepreneurs and creators.');
                try {
                    const parsed = JSON.parse(result.content.replace(/^```json?\n?/, '').replace(/\n?```$/, '').trim());
                    const trends = parsed.map((t) => ({ ...t, platform: 'google', relevanceScore: 85 }));
    ...
    async getAiTrends() {
        ...
        if (cached)
            return JSON.parse(cached);
        if (this.gemini.available) {
            const result = await this.gemini.searchAndSummarize(`What are the latest AI and machine learning developments, model releases, and tools launched in the past week?
    ...
    async getTrendsByNiche(niche) {
        ...
        let aiSummary = '';
        if (this.gemini.available) {
            const result = await this.gemini.searchAndSummarize(`Find the top 8 trending news and developments about "${niche}" relevant to the Nigerian market.
        Return as JSON: { "trends": [{ "title", "description", "url", "category", "relevanceScore" }], "summary": "2-3 sentence overview" }
        ONLY return valid JSON.`, `You are a Nigerian market research analyst specializing in ${niche}.`);
            try {
                const parsed = JSON.parse(result.content.replace(/```json?\n?/g, '').replace(/```/g, '').trim());
        ...
        const summary = { trends, aiSummary, fetchedAt: new Date().toISOString(), topic: niche };
        await this.redis.setex(cacheKey, this.CACHE_TTL, JSON.stringify(summary));
        return summary;
    }
    async generateContentIdeas(count = 10) {
        ...
        const result = await this.gemini.chat(`You are an editor at AmeboGist, a Nigerian tech and entrepreneurship platform.
       Generate viral content ideas that Nigerian entrepreneurs and creators will engage with.`, `Based on these trending topics:
${trendContext}
...
ONLY return the JSON array.`, { jsonMode: true, temperature: 0.85 });
        try {
            const ideas = JSON.parse(result.content);
            await this.redis.setex(cacheKey, 3600 * 6, JSON.stringify(ideas));
            return ideas;
        }
        catch {
            return this.getStaticContentIdeas();
    ...
    async fetchFromRssFeeds(categories, maxItems = 20) {
        const results = [];
        await Promise.allSettled(NIGERIAN_RSS_FEEDS.map(async ({ url, platform, name }) => {
            try {
    ...
    parseRssXml(xml, platform, source) {
        const items = [];
    ...
    async fetchGoogleTrendsNG() {
        try {
            ...
            return items.map((item) => ({ ...item, category: 'tech' }));
        }
        catch {
            return [];
        }
    }
    async summarizeTrends(trends) {
        ...
        const trendList = trends
            ...
            .join('\n');
        const result = await this.gemini.chat('You are a Nigerian tech trend analyst. Be concise and focus on business relevance.', `Analyze these trends and return JSON: {
...
Return ONLY the JSON object.`, { jsonMode: true, temperature: 0.3 });
        try {
    ...
    categorize(content) {
        if (/\bai\b|machine learning|gpt|gemini|llm|neural|openai/.test(content))
            return 'ai';
        if (/startup|funding|vc|venture|series [ab]|raise/.test(content))
            return 'startup';
        if (/bitcoin|crypto|blockchain|web3|defi/.test(content))
            return 'crypto';
        if (/policy|regulation|cbn|sec|nitda|nafdac|law/.test(content))
            return 'policy';
        if (/business|revenue|profit|market|sales/.test(content))
            return 'business';
        return 'tech';
    }
    scoreRelevance(content) {
        ...
        let score = 50;
        for (const kw of nigeriaKeywords)
            if (content.includes(kw))
                score += 10;
        for (const kw of boldmindKeywords)
            if (content.includes(kw))
                score += 5;
    ...
    getStaticContentIdeas() {
        return [
            { title: 'How to Start a Digital Business in Nigeria With ₦0', angle: 'Zero-cost tools for Nigerian founders', language: 'english', category: 'business', estimatedEngagement: 'high' },
            { title: 'ChatGPT vs Gemini: Which One Better for Naija Business?', angle: 'Pidgin comparison for Nigerian SME owners', language: 'pidgin', category: 'tech', estimatedEngagement: 'high' },
            { title: '10 WhatsApp Business Tricks Every Nigerian SME Owner Needs', angle: 'Actionable automation for informal sector', language: 'pidgin', category: 'business', estimatedEngagement: 'high' },
            { title: 'How Nigerian Creators Are Making ₦500k/Month on Instagram', angle: 'Real case studies, strategies', language: 'english', category: 'creator', estimatedEngagement: 'high' },
            { title: 'Best Free AI Tools Wey Go Help Your Business in 2026', angle: 'Free alternatives specifically tested in Nigeria', language: 'pidgin', category: 'tech', estimatedEngagement: 'medium' },
        ];
    }
};
