...
interface StudentSidebarProps { open?: boolean; onClose?: () => void; }
    ...
    >
      <div className="flex items-center justify-between h-16 px-5 flex-shrink-0"
        ...
        <Link href="/dashboard" className="flex items-center gap-2.5 no-underline">
          ...
          <span className="font-black text-white text-sm tracking-tight">EduCenter</span>
        </Link>
        <button onClick={onClose} className="md:hidden p-1.5 rounded-lg text-white/70 hover:text-white hover:bg-white/15">
          <X size={16} />
        </button>
      </div>
        ...
        {NAV.map(item => {
          ...
          return (
            ...
            >
              ...
              {isActive && <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: 'var(--product-primary)' }} />}
            </Link>
          );
        })}
      </nav>
      ...
      <div className="flex-shrink-0 border-t px-3 pb-3 pt-2 space-y-1" style={{ borderColor: 'var(--product-muted)' }}>
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl" style={{ backgroundColor: 'var(--product-muted)' }}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px]" style={{ color: 'var(--product-foreground)', opacity: 0.5 }}>Student</p>
          </div>
        </div>
