...
exports.BOLDMIND_PRODUCTS = void 0;
...
exports.BOLDMIND_PRODUCTS = [
    // ✅ LIVE PRODUCTS
    {
        id: 'prod_001',
        name: 'AmeboGist.ng',
        description: 'Pidgin news, lifestyle, and tech stories for Nigerian audience',
        category: 'media',
        status: 'live',
        version: '1.0.0',
        slug: 'amebogist',
        ...
        users: 10000,
        techStack: ['Next.js', 'MongoDB', 'PWA', 'Tailwind CSS'],
        teamSize: 2,
        timeline: 'Launched Q2 2025',
        priority: 1,
        integrations: ['Google AdSense', 'Meta API'],
        tags: ['news', 'pidgin', 'nigeria', 'content'],
        links: {
            website: 'https://amebogist.ng',
        },
        features: [
            'Pidgin news content',
            'Tech and lifestyle categories',
            'Mobile-first PWA',
            'Google AdSense integration',
            'SEO optimized'
        ],
        challenges: ['Content moderation', 'Ad revenue fluctuations'],
        opportunities: ['Video content', 'User-generated content', 'Mobile app'],
    ...
    },
    {
        ...
        description: 'Online courses and educational resources with JAMB past questions',
        category: 'education',
        status: 'live',
        version: '1.0.0',
        slug: 'educenter',
    ...
    },
    {
        ...
        monthlyRevenue: 20000, // ₦20k from 1 client
        users: 1,
    ...
    // 🔨 IN DEVELOPMENT
    {
        ...
        priority: 4,
        dependencies: ['PlanAI Suite'],
        integrations: ['Pictory AI', 'YouTube', 'Instagram', 'Facebook', 'Twitter', 'TikTok'],
    ...
    // MOBILE APPS (TWA - Trusted Web Activities)
    {
        id: 'prod_027',
        name: 'AmeboGist TWA',
...
function getProductBySlug(slug) {
    return exports.BOLDMIND_PRODUCTS.find(function (product) { return product.slug === slug; });
}
function getProductsByStatus(status) {
    return exports.BOLDMIND_PRODUCTS.filter(function (product) { return product.status === status; });
}
function getProductsByCategory(category) {
    return exports.BOLDMIND_PRODUCTS.filter(function (product) { return product.category === category; });
}
function getProductsByPriority(minPriority, maxPriority) {
    if (maxPriority) {
        return exports.BOLDMIND_PRODUCTS.filter(function (product) {
            return product.priority >= minPriority && product.priority <= maxPriority;
        });
    }
    return exports.BOLDMIND_PRODUCTS.filter(function (product) { return product.priority >= minPriority; });
}
function getLiveProducts() {
    return getProductsByStatus('live');
}
function getBuildingProducts() {
    return getProductsByStatus('building');
}
function getPlannedProducts() {
    return getProductsByStatus('planned');
}
function getConceptProducts() {
    return getProductsByStatus('concept');
}
// Revenue calculations
function calculateTotalMonthlyRevenue() {
    return exports.BOLDMIND_PRODUCTS.reduce(function (total, product) {
        return total + (product.monthlyRevenue || 0);
    }, 0);
}
function calculateProjectedRevenue(months) {
    if (months === void 0) { months = 12; }
    var liveRevenue = getLiveProducts().reduce(function (total, product) {
        return total + (product.monthlyRevenue || 0) * months;
    }, 0);
    // Estimate growth for building/planned products
    var buildingRevenue = getBuildingProducts().length * 100000 * months * 0.5; // 50% chance of launch
    var plannedRevenue = getPlannedProducts().length * 50000 * months * 0.3; // 30% chance of launch
    return liveRevenue + buildingRevenue + plannedRevenue;
}
// Team size calculations
function calculateTotalTeamSize() {
    var uniqueTeamSizes = new Set();
    exports.BOLDMIND_PRODUCTS.forEach(function (product) {
        if (product.teamSize)
            uniqueTeamSizes.add(product.teamSize);
    });
    return Array.from(uniqueTeamSizes).reduce(function (total, size) { return total + size; }, 0);
}
// Timeline analysis
function getUpcomingReleases(months) {
    ...
    return exports.BOLDMIND_PRODUCTS.filter(function (product) {
        ...
        var timelineMatch = product.timeline.match(/Q(\d) (\d{4})/);
        if (timelineMatch) {
            var quarter = parseInt(timelineMatch[1]);
            var year = parseInt(timelineMatch[2]);
            var releaseDate = new Date(year, (quarter - 1) * 3, 1);
...
function getProductDependencies(productSlug) {
    var product = getProductBySlug(productSlug);
    if (!product || !product.dependencies)
        return [];
    return product.dependencies
        .map(function (depSlug) { return getProductBySlug(depSlug); })
        .filter(function (dep) { return dep !== undefined; });
}
function getProductStatusSummary() {
    var liveProducts = getLiveProducts();
    var buildingProducts = getBuildingProducts();
    var plannedProducts = getPlannedProducts();
    var conceptProducts = getConceptProducts();
    return {
        total: exports.BOLDMIND_PRODUCTS.length,
...
exports.default = exports.BOLDMIND_PRODUCTS;
