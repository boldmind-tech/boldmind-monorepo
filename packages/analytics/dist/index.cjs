...
var BoldMindFlywheel = class {
	constructor(socialConfig) {
		...
		this.social = new _boldmind_utils.SocialIntegration(socialConfig);
	...
	async triggerFlywheelActions(userId, currentProduct, action) {
		...
		if (!journey) return;
		const product = _boldmind_utils.BOLDMIND_PRODUCTS.find((p) => p.slug === currentProduct);
		if (!product) return;
		switch (action) {
			case "view":
...
var CrossAppTracker = class CrossAppTracker {
	...
	getRecommendedProducts(userId) {
		...
		userProducts.forEach((productSlug) => {
			const product = _boldmind_utils.BOLDMIND_PRODUCTS.find((p) => p.slug === productSlug);
			if (product) {
				_boldmind_utils.BOLDMIND_PRODUCTS.filter((p) => p.category === product.category && p.slug !== productSlug).forEach((p) => recommendations.add(p.slug));
				product.tags.forEach((tag) => {
					_boldmind_utils.BOLDMIND_PRODUCTS.filter((p) => p.tags.includes(tag) && p.slug !== productSlug).forEach((p) => recommendations.add(p.slug));
				});
			}
		});
...
//#endregion
exports.BoldMindFlywheel = BoldMindFlywheel;
exports.CrossAppTracker = CrossAppTracker;
exports.Events = Events;
exports.analytics = analytics;
exports.crossAppTracker = crossAppTracker;
exports.mixpanel = mixpanel;
exports.posthog = posthog;
exports.runFlywheelExample = runFlywheelExample;
exports.trackEvent = trackEvent;
exports.trackPageView = trackPageView;
exports.trackPurchase = trackPurchase;
exports.trackSubscription = trackSubscription;
exports.trackUserSignup = trackUserSignup;
